...
public class JDBCMessageDAO implements MessageDAO {
	...
	public void deletePublicMessage(String messageId) {
		jdbcTemplate.execute("START TRANSACTION;");
		String deleteSelectedPublicMessages = "DELETE "+
				 "FROM message WHERE private = FALSE "+
				 "AND message_id = ?";
				int result = jdbcTemplate.update(deleteSelectedPublicMessages, Integer.parseInt(messageId));
//				SqlRowSet results = jdbcTemplate.queryForRowSet(deleteSelectedPublicMessages, messageId);
				if(result >= 1) {
