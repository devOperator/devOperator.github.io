...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		for(int i = 0; i < nums.length - 1; i++) { //remember .length will put you out of bounds
	...
	public int[] maxEnd3(int[] nums) {
		...
		if(nums[nums.length - 1] > nums[0]) {
			maxArray[0] = nums[nums.length - 1];
			maxArray[1] = nums[nums.length - 1];
			maxArray[2] = nums[nums.length - 1];
		}
		else {
			maxArray[0] = nums[0];
			maxArray[1] = nums[0];
	...
	public int sum13(int[] nums) {
		...
		for(int i = 0; i < nums.length; i++) {
			if(nums[i] == 13) {
				if(i <= nums.length - 2) {
					i = i + 2; //skipping the next number in array;
				}
			}
			else {
				sum += nums[i];
