...
SELECT name, population from city
WHERE district = 'Montana' AND countrycode = 'USA';
...
SELECT name, continent, population, gnp FROM country
...
WHERE countrycode = 'USA' AND population BETWEEN 1000000.00 AND 2000000.00;

-- 14. The name and region of all countries in North or South America except for countries in the Caribbean (27 rows)

SELECT name, region from country 
WHERE region IN ('North America', 'South America') AND region NOT IN ('Caribbean'); -- only 19 rows
...
WHERE gnp > 1000000.00 AND population < 100000000;
