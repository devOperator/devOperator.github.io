...
// this line declares a variable named `numberOfDaysPerWeek` and assigns it the numeric value 3
...
// Javascript is a **dynamically typed** language. The following code does not cause an error.
...
// Now go back and try switching numberOfQuarterBacks to "nineteen" and see what happens 
// when we use ++
// We'll get NaN
...
Strict equality **`===`** or **`!==`** compares two values for equality.
Neither value is implicitly converted to another value before comparison.
*/
var num = 0;
...
A unary operator only has one operand. 
The most common unary operator is `typeof`.
The only operand supplied is the value that you want to get the type of.
*/
...
This is more meant to show some of the pitfalls and dangers of not paying attention
to variable scope
*/
...
/* Default Parameters
In Javascript rather than checking if your parameters are undefined before
assigning them a default value, you can use default parameters.
*/
