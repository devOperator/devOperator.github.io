...
public class Exercises {
	...
	/*
	 6. Given 2 integers, a and b, return true if one if them is 10 or if their sum is 10.
	 makes10(9, 10) → true
	 makes10(9, 9) → false
	 makes10(1, 9) → true
	 */
	...
	/*
	 14. Given 2 integer values, return true if they are both in the range 30..40 inclusive, or they are both 
	...
	public int caughtSpeeding(int speed, boolean isBirthday) {
		int result = 0;
		int birthdaySpeedDelta = isBirthday? 5 : 0; 
		
		
		if(speed <= 60 + birthdaySpeedDelta) {
			result = 0;
		}
		if(speed >= 61 + birthdaySpeedDelta && speed <= 80 + birthdaySpeedDelta) {
			result = 1;
		}
		
		if(speed > 81 + birthdaySpeedDelta) {
			result = 2;
		}
		return result;
	}
	...
	public boolean nearTen(int num) {
		...
		if (num <= 10) {
			...
			if(num % 10 <= 2 || (10 - (num % 10)) <= 2) { //first condition for numbers with % 10 just over a multiple, 2nd for those just under
				result = true;
			}
		}
		return result;
