...
public class Exercises {
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		List<String> fizzValueList = new ArrayList<>();
		for(Integer number : integerArray) {
			if(number % 3 == 0) {
				if(number % 5 == 0) {
					fizzValueList.add("FizzBuzz");
				}else{
					fizzValueList.add("Fizz");
				}
			}else if (number % 5 == 0) {
				fizzValueList.add("Buzz");
			}else {
				fizzValueList.add(number.toString());
			}
		}
		return fizzValueList;
	}
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		valSet.addAll(stringList);
		stringList.removeAll(stringList);
		stringList.addAll(valSet);
		return stringList;
	}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		for(int i = 0; i < seatNumberList.size(); i++) {
			...
			separatedList.add(seatNumberList.get(i));
		}
		for(int i = 0; i < seatNumberList.size(); i++) {
